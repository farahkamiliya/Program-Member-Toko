...
def show_login_admin():
    login_admin_frame = tk.Frame(root, bg="#F5C400")  # Mengatur latar belakang frame menjadi kuning
    login_admin_frame.grid(row=0, column=0, padx=10, pady=10, sticky="nsew")
    
    tk.Label(login_admin_frame, text="Login Admin", bg="#F5C400", font=("segoe UI", 45, "bold")).grid(row=0, column=1, pady=(150,20), padx=150, sticky="w")
    from PIL import Image
    img = Image.open(r"C:\Users\HP\projecta\envprojecta\logo.png")
    img = img.resize((683, 149))
    img_tk = ImageTk.PhotoImage(img)
    label_img = tk.Label(login_admin_frame, image=img_tk, bg="#F5C400")
    ...
    tk.Label(login_admin_frame, text="No HP:", bg='#F5C400', font=("segoe UI", 20)).grid(row=1, column=1, pady=10, padx=20, sticky='E' + 'W' + 'S')
    entry_no_hp = tk.Entry(login_admin_frame, width=10, font=("segoe UI", 20))
    entry_no_hp.grid(row=2, column=1, sticky='E' + 'W')

    tk.Label(login_admin_frame, text="Password:", bg='#F5C400', font=("segoe UI", 20)).grid(row=3, column=1, padx=20, pady=10, sticky='E' + 'W')
    entry_password = tk.Entry(login_admin_frame, width=10,  font=("segoe UI", 20), show="*")
    entry_password.grid(row=4, column=1, sticky='E' + 'W')

    Login_admin_button = tk.Button(login_admin_frame, text="Login", bg='#102A71', fg='white', font=("segoe UI", 20), command=login_admin)
    Login_admin_button.grid(row=5, column=1, pady=20, padx=20)

    switch_frame(login_admin_frame)
...
def show_menu():
    ...
    tk.Label(menu_frame, text="Menu Utama", bg="#F5C400", font=("segoe UI", 45, "bold")).grid(row=0, column=1, pady=(150, 20), padx=150, sticky="w")
    from PIL import Image
    img = Image.open(r"C:\Users\HP\projecta\envprojecta\logo.png")
    img = img.resize((683, 149))
    img_tk = ImageTk.PhotoImage(img)
    ...
    Daftar_member = tk.Button(menu_frame, text="Daftar Member", bg='#102A71', fg='white', font=("segoe UI", 20), command=show_daftar_member)
    Daftar_member.grid(row=1, column=1, pady=10, padx=20)
    Login_member = tk.Button(menu_frame, text="Login Member", bg='#102A71', fg='white', font=("segoe UI", 20), command=lg.show_login_page)
    Login_member.grid(row=2, column=1, pady=10, padx=20)
    Keluar = tk.Button(menu_frame, text="Keluar", bg='#102A71', fg='white', font=("segoe UI", 20), command=root.quit)
    Keluar.grid(row=3, column=1, pady=10, padx=20)
